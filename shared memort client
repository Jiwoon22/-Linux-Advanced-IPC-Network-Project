...
char quiz_list[3][50] = { "Love is Timing","LOVE","Clown" };
int idx = 0, tidx = 0;
char song[2][100] = { "play 10cm.wav","play Leesang.wav" };

int main(){
  pthread_t thread_1,thread_2;
  void* thread_1_result;
  void* thread_2_result;
  void* sm=(void*)0;
  ...
  shared_client1=(struct shared_use_st1*)sm;

  pthread_create(&thread_1,NULL,Recv,NULL);
  pthread_create(&thread_2,NULL,Send,NULL);
...
while (1)
{
	if (shared_client1->start_flag == 1)
	{
...
void* Send(){
  ...
  while(1){
    while(shared_client1->flag==0){
	while(win_flag)
	{
     		if (strcmp(buffer, quiz_list[idx]) == 0) 
     		{
			printf("Excellant!!\n");
	...
	{
		strncpy(shared_client1->message, buffer, BUFFER_SIZE); 
	}    
 	if((semid=initsem(semkey1))<0)
 	      exit(1);
